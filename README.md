...
# 🚢 Roto-LLM-Agent: On-Device Rotating Machinery Fault Diagnosis System
...
```mermaid
graph TD
    User[User / Operator] -->|Natural Language| LLM[LLM Agent (Qwen2.5-14B)]
    
    subgraph "Edge Computing Unit (Local)"
        LLM -->|Function Call| Router[Tool Router]
        
        Router -->|Read| Tool1[Data Previewer]
        Router -->|Plot| Tool2[Signal Visualizer]
        Router -->|Infer| Tool3[Diagnosis Model (PyTorch)]
        
        Tool1 --> Data[(CSV Metadata)]
        Tool2 --> Signal[(NPZ Signals)]
        Tool3 --> Checkpoint[(Model Weights)]
        
        Tool1 & Tool2 & Tool3 -->|Structured Output| LLM
    end
    
    LLM -->|Reasoning & Report| User
